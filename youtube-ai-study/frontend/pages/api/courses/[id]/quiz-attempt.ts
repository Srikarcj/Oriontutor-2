...
import { quizAttemptSchema } from "../../../../lib/validation/schemas";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const limit = applyRateLimit(req, res, { windowMs: 60_000, max: 30, keyPrefix: "courses-quiz-attempt" });
  ...
  const upstream = await fetch(`${backendUrl}/api/courses/${encodeURIComponent(courseId)}/quiz-attempt`, {
    method: "POST",
    headers: {
  ...
  if (!upstream.ok) return res.status(upstream.status).json({ error: text || "Quiz attempt failed" });

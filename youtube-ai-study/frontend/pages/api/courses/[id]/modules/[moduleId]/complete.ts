...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const limit = applyRateLimit(req, res, { windowMs: 60_000, max: 30, keyPrefix: "courses-module-complete" });
  ...
  const upstream = await fetch(
    `${backendUrl}/api/courses/${encodeURIComponent(courseId)}/modules/${encodeURIComponent(moduleId)}/complete`,
    {
      method: "POST",
      headers: {
        "X-User-Id": userId,
  ...
  if (!upstream.ok) return res.status(upstream.status).json({ error: text || "Failed to complete module" });

...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const limit = applyRateLimit(req, res, { windowMs: 60_000, max: 60, keyPrefix: "leaderboard" });
  if (!limit.allowed) return res.status(429).json({ error: "Too many requests" });

  const backendUrl = readRequired("BACKEND_URL");
  const upstream = await fetch(`${backendUrl}/api/leaderboard`, {
    method: "GET",
    headers: {
  ...
  if (!upstream.ok) return res.status(upstream.status).json({ error: text || "Failed to load leaderboard" });

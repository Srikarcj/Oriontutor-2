...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const limit = applyRateLimit(req, res, { windowMs: 60_000, max: 60, keyPrefix: "dashboard" });
  ...
  const upstream = await fetch(`${backendUrl}/api/dashboard`, {
    method: "GET",
    headers: {
      "X-User-Id": userId,
  ...
  if (!upstream.ok) return res.status(upstream.status).json({ error: text || "Failed to load dashboard" });

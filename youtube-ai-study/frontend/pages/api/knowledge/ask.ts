...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const limit = applyRateLimit(req, res, { windowMs: 60_000, max: 40, keyPrefix: "knowledge-ask" });
  ...
  const upstream = await fetch(`${backendUrl}/api/knowledge/ask`, {
    method: "POST",
    headers: {
  ...
  if (!upstream.ok) return res.status(upstream.status).json({ error: text || "Knowledge ask failed" });
